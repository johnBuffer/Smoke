...
struct DroneRenderer
{
	...
	void draw(const Drone::Thruster& thruster, const Drone& drone, RenderContext& target, sf::Color color, bool right)
	{
		...
		const float push_length = MathVec2::length(body_to_thruster);
		const float push_angle = MathVec2::angle(body_to_thruster) + Math::PI;
		...
		const float push_length_2 = MathVec2::length(body_to_thruster_2);
		const float push_angle_2 = MathVec2::angle(body_to_thruster_2) + Math::PI;
	...
	void draw(const Drone& drone, RenderContext& target, sf::Color color = sf::Color::White)
	{
		// Draw body
	...
	void drawBody(const Drone& drone, const sf::Color& color, RenderContext& target)
	{
		...
		const float da = 2.0f * Math::PI / float(quality);
